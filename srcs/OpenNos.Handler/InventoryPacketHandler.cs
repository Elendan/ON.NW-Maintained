    ...
    public class InventoryPacketHandler : IPacketHandler
    {
        ...
        public void Deposit(DepositPacket depositPacket)
        {
            ...
            if (depositPacket.NewSlot >= (depositPacket.PartnerBackpack
                ? (Session.Character.StaticBonusList.Any(s => s.StaticBonusType == StaticBonusType.PetBackPack || Session.Character.Authority >= AuthorityType.VipPlusPlus) ? 50 : 0)
        ...
        public void EquipmentInfo(EquipmentInfoPacket equipmentInfoPacket)
        {
                ...
                case 7:
                    ...
                    switch (equipmentInfoPacket.Slot)
                    {
                        case 2:
                            ...
                            break;
                        case 3:
                            invType = InventoryType.ThirdPartnerInventory;
                            break;
                    }
                    inventory = value != 12 ? Session.Character.Inventory.LoadBySlotAndType<WearableInstance>(value, invType) : Session.Character.Inventory.LoadBySlotAndType<SpecialistInstance>(value, invType);
                    break;
            ...
            Session.SendPacket(inventory.Item.EquipmentSlot != EquipmentType.Sp ? inventory.GenerateEInfo() :
                inventory.Item.SpType == 0 && inventory.Item.ItemSubType == 4 ? (inventory as SpecialistInstance)?.GeneratePslInfo() : (inventory as SpecialistInstance)?.GenerateSlInfo());
        }
                ...
                switch (exchangeRequestPacket.RequestType)
                {
                    case RequestExchangeType.Requested:
                        ...
                        else
                        {
                            ...
                            {
                                Session.SendPacket(UserInterfaceHelper.Instance.GenerateModal(string.Format(Language.Instance.GetMessageFromKey("YOU_ASK_FOR_EXCHANGE"), targetSession.Character.Name),
                                    0));
                            ...
                            lock(targetSession.Character.Inventory)
                            {
                                lock(Session.Character.Inventory)
                                {
                                    ...
                                    if (Session.Character.ExchangeInfo.Validate && targetExchange.Validate)
                                    {
                                        ...
                                        {
                                            Session.SendPacket(UserInterfaceHelper.Instance.GenerateInfo(string.Format(Language.Instance.GetMessageFromKey("IN_WAITING_FOR"),
                    ...
                    case RequestExchangeType.List:
                        ...
                        if (!Session.Character.InExchangeOrTrade || !otherInExchangeOrTrade)
                        {
                            ...
                            ServerManager.Instance.SetProperty(exchangeRequestPacket.CharacterId, nameof(Character.ExchangeInfo),
                                new ExchangeInfo { TargetCharacterId = Session.Character.CharacterId, Confirm = false });
                        ...
                        {
                            Session.CurrentMapInstance?.Broadcast(Session, UserInterfaceHelper.Instance.GenerateModal(Language.Instance.GetMessageFromKey("ALREADY_EXCHANGE"), 0),
                ...
                if (mapItem is MonsterMapItem item)
                {
                    ...
                    if (Session.CurrentMapInstance.MapInstanceType != MapInstanceType.LodInstance && monsterMapItem.OwnerId.HasValue && monsterMapItem.OwnerId.Value != -1)
                    {
                        ...
                        if (item.CreatedDate.AddSeconds(30) > DateTime.Now &&
                            !(monsterMapItem.OwnerId == Session.Character.CharacterId || group != null && group.SharingMode == (byte)GroupSharingType.Everyone))
                        {
                    ...
                    if (mapItemInstance.Item.ItemType == ItemType.Map)
                    {
                        if (mapItemInstance.Item.Effect == 71)
                        {
                            ...
                            Session.SendPacket(UserInterfaceHelper.Instance.GenerateMsg(
                                string.Format(Language.Instance.GetMessageFromKey("SP_POINTSADDED"), mapItem.GetItemInstance().Item.EffectValue), 0));
                            ...
                            if (inv != null)
                            {
                                ...
                                if (Session.CurrentMapInstance.MapInstanceType == MapInstanceType.LodInstance)
                                {
                                    Session.CurrentMapInstance?.Broadcast(Session.Character.GenerateSay(
                                        $"{string.Format(Language.Instance.GetMessageFromKey("ITEM_ACQUIRED_LOD"), Session.Character.Name)}: {inv.Item.Name} x {mapItem.Amount}", 10));
                                }
                            }
                            else
                            {
        ...
        public void MoveItem(MviPacket mviPacket)
        {
            lock(Session.Character.Inventory)
            {
                ...
                // actually move the item from source to destination
                Session.Character.Inventory.MoveItem(mviPacket.InventoryType, mviPacket.InventoryType, mviPacket.Slot, (ushort)mviPacket.Amount, mviPacket.DestinationSlot,
                    out ItemInstance previousInventory, out ItemInstance newInventory);
        ...
        public void Remove(RemovePacket removePacket)
        {
            ...
            ItemInstance inventory = removePacket.InventorySlot != (byte)EquipmentType.Sp
                ? Session.Character.Inventory.LoadBySlotAndType<WearableInstance>(removePacket.InventorySlot, equipment)
                : Session.Character.Inventory.LoadBySlotAndType<SpecialistInstance>(removePacket.InventorySlot, equipment);
            if (inventory == null)
            {
                return;
            }
        ...
        public void Repos(ReposPacket reposPacket)
        {
            ...
            // actually move the item from source to destination
            Session.Character.Inventory.MoveItem(reposPacket.PartnerBackpack ? InventoryType.PetWarehouse : InventoryType.Warehouse,
                reposPacket.PartnerBackpack ? InventoryType.PetWarehouse : InventoryType.Warehouse, reposPacket.OldSlot, reposPacket.Amount, reposPacket.NewSlot, out ItemInstance previousInventory,
            ...
            Session.SendPacket(previousInventory != null
                ? (reposPacket.PartnerBackpack ? previousInventory.GeneratePStash() : previousInventory.GenerateStash())
                : (reposPacket.PartnerBackpack ? UserInterfaceHelper.Instance.GeneratePStashRemove(reposPacket.OldSlot) : UserInterfaceHelper.Instance.GenerateStashRemove(reposPacket.OldSlot)));
        }
        ...
        public void SpTransform(SpTransformPacket spTransformPacket)
        {
            ...
            if (spTransformPacket.Type == 10)
            {
                ...
                if (CharacterHelper.Instance.SpPoint(specialistInstance.SpLevel, specialistInstance.Upgrade) - specialistInstance.SlDamage - specialistInstance.SlHP - specialistInstance.SlElement -
                    specialistInstance.SlDefence - specialistDamage - specialistDefense - specialistElement - specialistHealpoints < 0)
                {
                    return;
                }
